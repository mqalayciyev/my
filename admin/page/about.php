...
if($action === "edit"){
    ...
    $about = mysqli_fetch_assoc(mysqli_query($conn, $sql));
    $text = $about['text'];
    $id = $_GET['id'];
}
            ...
            <div id="file" class="col-12">
                <form method="POST" name="13" novalidate="novalidate" enctype="multipart/form-data">
                    <div class="col-12 border rounded p-3">
                        ...
                        <button id="cancel" type="reset" class="btn btn-danger"><a style="color: white; text-decoration: none;" href="admin.php?page=about">Ləğv et</a></button>
                    </div>
                </form>
            </div>
            <div class="col-12 my-3">
            <?php
                    $sql = "SELECT * FROM about ORDER BY `status` DESC";
                    ...
                    while($res = mysqli_fetch_assoc($query)){
                        ...
                        <div class="row">
                            <div class="col-4 border-right px-0">
                                <i onclick="rotateIMG('.$res['id'].', event)" class="fas fa-arrows-alt-h text-white fa-2x col-12 p-1 text-center bg-primary"></i>
                                <img src="../media/about/'.$res['image'].'" style="transform: rotateY('.$res['rotate'].')" class="w-100" alt="'.$res['image'].'"">
                            </div>
                            <div class="col-7">
                                <p>'.$res['text'].'</p>
                            </div>
                            <div class="col-1 border-left">
                                <div class="row align-items-center justify-content-center h-100">
                                    <ul class="list-inline m-0 p-0 text-center">
                                        <li class="text-justify"><a href="config.php?query=delete_about_info&id='.$res['id'].'"><i class="fas fa-trash fa-2x"></i></a></li>
                                        <li class="text-justify my-3"><a href="config.php?query=change_about_info_status&id='.$res['id'].'"><i class="far fa'.$check.'-square fa-2x"></i></a></li>
                                        <li class="text-justify"><a href="admin.php?page=about&action=edit&id='.$res['id'].'"><i class="fas fa-edit fa-2x"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
